...
#pragma mark - 声明
@interface FMBudgetKeyboard()<UITextFieldDelegate>
...
#pragma mark - 实现
@implementation FMBudgetKeyboard

+ (instancetype)init {
    FMBudgetKeyboard *view = [FMBudgetKeyboard loadFirstNib:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_WIDTH / 5 * 4 + SafeAreaBottomHeight)];
    [view setHidden:YES];
    [view initUI];
    return view;
}
...
- (void)calculationClick:(UIButton *)btn {
    ...
    if ([btn.titleLabel.text isEqualToString:@"完成"]) {
        if (self.budgetMonitor) {
            if (self.model) {
                self.budgetMonitor(_moneyLab.text, self.currentDate, self.model.budgetID);
            }else {
                    self.budgetMonitor(_moneyLab.text, self.currentDate, nil);
            }
        }
    }
}
...
- (void)setUpKeyBoardWithBudget:(FMBudget *)budget {
    ...
    NSString *key;
    if (budget) {
        self.currentDate = budget.date;
        key = [NSString stringWithFormat:@"%ld-%02ld-%02ld", budget.date.year, budget.date.month, budget.date.day];
    } else {
        NSDate *now = [NSDate date];
        key = [NSString stringWithFormat:@"%ld-%02ld-%02ld", now.year, now.month, now.day];
        self.currentDate = now;
    }
    [self setMoney:[budget.amount description].mutableCopy];
...
- (void)monitoringCreateBudget:(budgetMonitorBlock)budgetMonitor {
    self.budgetMonitor = budgetMonitor;
}
@end
