...
#pragma mark - 声明
@interface FMBillKeyboard()<UITextFieldDelegate>
...
@property (nonatomic, copy  ) billMonitorBlock billMonitor;
...
#pragma mark - 实现
@implementation FMBillKeyboard


+ (instancetype)init {
    FMBillKeyboard *view = [FMBillKeyboard loadFirstNib:CGRectMake(0, SCREEN_HEIGHT, SCREEN_WIDTH, SCREEN_WIDTH / 5 * 4 + SafeAreaBottomHeight)];
    [view setHidden:YES];
    [view initUI];
    return view;
}
...
- (void)calculationClick:(UIButton *)btn {
    ...
    if ([btn.titleLabel.text isEqualToString:@"完成"]) {
        if (self.billMonitor) {
            if (self.model) {
                self.billMonitor(_moneyLab.text, _markField.text, self.currentDate, self.model.billID);
            }else {
                    self.billMonitor(_moneyLab.text, _markField.text, self.currentDate, nil);
            }
        }
    }
}
...
- (void)setUpKeyBoardWithBill:(FMBill *)bill {
    ...
    NSString *key;
    if (bill) {
        self.currentDate = bill.date;
        key = [NSString stringWithFormat:@"%ld-%02ld-%02ld", bill.date.year, bill.date.month, bill.date.day];
    } else {
        NSDate *now = [NSDate date];
        key = [NSString stringWithFormat:@"%ld-%02ld-%02ld", now.year, now.month, now.day];
        self.currentDate = now;
    }
    [self.markField setText:bill.remarks];
    [self setMoney:[bill.amount description].mutableCopy];
